...
echo                installation is complete. When you get to the desktop, WMI will run the last time.
echo.
ECHO.
echo ===================================================================================================
...
echo        "Getting Windows ready..." loading screen or something like that. Wait for
echo        the PC to restart, and then you should see the lockscreen with your user account.
echo        Log in and then wait until the end of the prepairing process.
echo        Next, you'll have to set up privacy settings. After that, you're good to go^! WMI will
echo        start another time to clean the environment and to show the Credits screen.
echo.
...
echo %wil_%\wmi.bat >>"%wil_%\ProgramData\Microsoft\Windows\Start Menu\Programs\StartUp\temp.bat"
exit
...
echo Please wait...
echo ===================================================================================================
if exist "%wil_%\stagenum.wmidat" del /f /s /q "%wil_%\stagenum.wmidat" > nul
if exist "%wil_%\ProgramData\Microsoft\Windows\Start Menu\Programs\StartUp\temp.bat" del /f /s /q "%wil_%\ProgramData\Microsoft\Windows\Start Menu\Programs\StartUp\temp.bat" > nul
...
echo.
echo        Including WMI Rollback 0.1.5 UEFI edition
echo.
...
cls
color
title WMI Rollback 0.1.5
mode 100,30
echo.

echo   ################################ WMI Rollback version 0.1.5 UEFI ###############################
echo   ###                                                                                          ###
echo   ###    Welcome to WMI Rollback 0.1.5. There seems to be a problem with the WMI installing    ###
...
if "%problem%"=="wsnc" goto :wsnc
	...
	(echo select disk %destdisk% &echo sel par 1 &echo format fs=ntfs quick) | diskpart 
	echo.
	...
	echo.
	echo  You started WMI Rollback from the command line. Select the disk you want to rescue.
	echo.
	:dpdsp
	echo lis dis >"%mypath:~0,-1%\dpscript.txt"
	echo        Look at the number of the disk you want to recovery:      
	echo.
	(echo lis dis)  | diskpart &echo.
	echo.
	echo.
	:tempbookmark3
	set /p destdisk=       Enter the number [just the number] of the disk you want to recovery:
	FOR /F "tokens=2" %%a IN ('diskpart /s dpscript.txt') DO (
		if "%%a"=="%destdisk%" (
...
echo   ###                                                                                          ###
echo   ###    NOTE: the UEFI edition of WMI Rollback 0.1.5 doesn't support the use of WMI after     ###
...
echo   ###                                                                                          ###
echo   ###                 To ensure that your drive is now ready for a next usage,                 ###
echo   ###                              press ENTER to reboot your PC.                              ###
echo   ###                                                                                          ###
echo   ####################### A WMI dependency - not distributable separately. #######################
echo.
echo.
