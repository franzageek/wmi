...
::"%mypath:~0,-1\"
title Windows Manual Installer 0.1.4 - Stage %_stage%
cls
echo.
echo    *************************************
...
set /a att_times=0
	...
	echo.
	cls
	echo Now, we'll execute a command that scans Windows installations on the disk.
	echo If the number increases by 1, then IT WORKED^^!
	...
	echo.
	:tempbookmark2
	set /p yonbr_s_os=Is the number of Windows installations increased by 1? [Y/N]:
	if /i "%yonbr_s_os%"=="y" (goto :oktoproceed)
	if /i "%yonbr_s_os%"=="n" ( 
		echo [ERROR]: Something went wrong when applying the image.
		echo Try rebooting and restarting the program [changes to the disk will be kept for DiskPart].
		pause
		exit
	)
	goto :tempbookmark2
	...
	bootrec /scanos
	echo.
	echo This time, if the number of installation is decreased by 1, then all is set and good to go.
	:tempbookmark3
	set /p ypbrt=Has the number of Windows installations returned to the previous value [decreased by 1]? [Y/N]:
	if /i "%ypbrt%"=="y" (goto :yestoproceed)
	if /i "%ypbrt%"=="n" ( 
		echo [ERROR]: Something went wrong when creating boot files
		echo Try rebooting and restarting the program [changes to the disk will be kept for DiskPart].
		pause
		exit
	)
	...
	:yestoproceed
	echo Perfect^^! Now we're gonna apply the changes to the Registry Editor.
	echo.
	...
	echo Adding CMDLINE...
	reg add "HKLM\SYS\Setup" /v "CmdLine" /f /t REG_SZ /d "C:\wmi.bat"
	goto :end_stage0
	...
	echo           *  should see a CMD window. WMI will automatically launch from     *
	echo           *  that CMD window. The WMI copy on your USB drive is gonna be     *
	echo           *  reset in order not to cause filename conflicts while  running   *
	echo           *  WMI.                                                            *                                     
	echo           *                                                                  *
...
title Windows Manual Installer 0.1.4 - End
echo Done.
cls
echo.
echo           ********************************************************************
...
echo           *                                                                  *
echo           *                                                                  *
echo           *  Windows Manual Installer version 0.1.4                          *
echo           *  Developed by ^<franzageek^>                                       *
echo           *                                                                  *
